...
if($type == 1){
	...
	$title="Food Type Wise";
	if ($_POST['food_type'] != 'Vegan') {
	    $squery = "select * from foodrec where foodtype='" . $_POST['food_type'] . "'";
	    $querysel = mysqli_query($con, $squery);
	} else {
	    $squery = "select * from foodrec where isvegan='yes'";
	    $querysel = mysqli_query($con, $squery);
	}
}elseif($type == 3){
	$title="Food Category Wise";
    $squery = "select * from foodrec where foodcat='" . $_POST['cat'] . "'";
    $querysel = mysqli_query($con, $squery);
}elseif($type == 4){
	$title="Diabetic Wise Food";
    $squery = "select * from foodrec where fordiabetic='" . $_POST['diabetic'] . "'";
    $querysel = mysqli_query($con, $squery);
}
