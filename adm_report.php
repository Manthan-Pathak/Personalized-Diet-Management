...
if($type == 1){
	$title="All Admin's Record";
    $squery = "select * from admin";
    $querysel = mysqli_query($con, $squery);

}elseif($type == 3){
	$title="Record based on Admin's Satus";
    $squery = "select * from admin where isactive='" . $_POST['isactive'] . "'";
    $querysel = mysqli_query($con, $squery);
}elseif($type == 2){
	$title="Record based on Admin's Id";
    $squery = "select * from admin where aid='" . $_POST['aid'] . "'";
    $querysel = mysqli_query($con, $squery);
}
