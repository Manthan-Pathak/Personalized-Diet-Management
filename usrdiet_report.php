...
if($type == 1){
	$title="All user's Diet Record";
    $squery = "select * from dietrec";
    $querysel = mysqli_query($con, $squery);

}elseif($type == 2){
	$title="Record based on user's Goal";
    $squery = "select * from dietrec where goal='" . $_POST['goal'] . "'";
    $querysel = mysqli_query($con, $squery);
}elseif($type == 3){
	$title="Record based on user's Id";
    $squery = "select * from dietrec where usrid='" . $_POST['usrid'] . "'";
    $querysel = mysqli_query($con, $squery);
}
